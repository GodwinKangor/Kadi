...
export function applyCard(game, playerId, cardIndices, declaredSuit, declaredRank) {
  ...
  if (QUESTIONS.has(card.rank)) {
    if (requested.length > 1) {
      ...
      // trusting it: everything must share the first card's suit, exactly
      // one selected card must be a valid closing Answer rank, and the
      // rest must all be Question cards, with nothing duplicated.
      const rest = requested.slice(1);
      const uniqueOk = new Set(requested.map((c) => c.id)).size === requested.length;
      const sameSuit = rest.every((c) => c.suit === card.suit);
      const closers = rest.filter((c) => QUESTION_CLOSING_RANKS.has(c.rank));
      const chainPart = rest.filter((c) => QUESTIONS.has(c.rank));
      const noStrays = chainPart.length + closers.length === rest.length;

      if (uniqueOk && sameSuit && noStrays && closers.length === 1) {
        answerCard = closers[0];
        playedCards = [card, ...chainPart, answerCard];
      } else {
        drawCards(game, playerIndex, 1);
        game.message = "That selection doesn't close the question — pick same-suit Question cards plus exactly one closing Answer card.";
        game.log = [`${player.name} tried an invalid question chain and picked one.`, ...game.log].slice(0, 8);
        advanceTurn(game, playerId, nextIndex(game.currentPlayer, game.direction, game.players.length));
        return;
      }
    } else {
