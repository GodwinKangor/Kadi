...
// same Answer rank, or a single-suit run of Question Cards closed by
// exactly one Answer Card of that suit.
export function canGoOutWithHand(hand) {
  if (hand.length === 0) return false;

  if (hand.every((c) => c.rank === hand[0].rank) && ANSWERS.has(hand[0].rank)) {
    return true;
  }

  const suit = hand[0].suit;
  if (hand.every((c) => c.suit === suit)) {
    const answerCount = hand.filter((c) => ANSWERS.has(c.rank)).length;
...
export function applyCard(game, playerId, cardIndex, declaredSuit, declaredRank) {
  ...
  if (card.rank === "J") {
    ...
    if (playedCards.length >= 2) {
      // Two or more Aces together lock both suit AND rank — a much
      // stricter requirement than a single Ace's suit-only lock.
      game.declaredRank = VALID_RANK_DECLARATIONS.has(declaredRank) ? declaredRank : null;
      message = `${player.name} played ${playedCards.length} Aces and locked suit and rank.`;
