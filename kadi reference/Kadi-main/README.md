...
- Playing two or more Aces together (see the same-rank bulk play rule above) is a much stronger move: the player locks in *both* a required suit (or color) and a required rank. The next player can only continue by playing an Ace (still wild), a card that satisfies both the locked suit and rank, or by drawing. A single countering Ace can only change the locked suit/color — the locked rank stays in force until either it's actually satisfied by a real card, or someone plays a fresh double-Ace (or more) to re-lock both.
